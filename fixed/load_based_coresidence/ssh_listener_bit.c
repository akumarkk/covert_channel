...
#define MAXIMUM_MEAN_AGE 25
/* Let the size of the data to be sufficiently large */
#define READ_DATA_SIZE     (1024)*(1024)*(128)

#define SAMPLE_SIZE_FOR_MEAN  2 
        ...
        struct timespec    end)
{
    ...
    if(diff.tv_sec > 0)
    {
	//debug("diff.tv_sec = %ld", diff.tv_sec);
        ...
        long    read_addr)
{
    ...
    time_t      start;
    long        sum = 0, tmp_avg, avg = -1, diff;
    long        elapsed_time[SAMPLE_SIZE_FOR_MEAN + 1]; // This will have elapsed time in microseconds.
    int         j, ret = -1, i=-1;
    //char        read_buff[READ_DATA_SIZE];
    //char        read_buff[1024];
    ...
    while(time(NULL) <= (start + SAMPLING_TIME))
    {
        ...
        //debug("time = %ld  Elapsed time     :   %ld ns", time(NULL), diff);
        if(i == SAMPLE_SIZE_FOR_MEAN)
        {
	    sum = 0;
            for(j=0; j <= SAMPLE_SIZE_FOR_MEAN; j++)
                sum = sum + elapsed_time[j];
	    
            sum = sum + diff;

            tmp_avg = sum/(SAMPLE_SIZE_FOR_MEAN + 2); //This is because diff + sample_size + 1
            if(avg == -1)
                avg = tmp_avg;
            else
                avg = (avg + tmp_avg)/2;
...
{
    int     i=0;
    long    prev = -1, avg_time, std_dev = 0;
    long    slope = -1;
    ...
    for(i=0; i < SAMPLING_COUNT; i++)
    {
        ...
        if(slope == -1)
            ...
            slope = abs(prev - avg_time);
	//debug("slope max diff :	%ld", (long)abs(prev - avg_time));
    }
